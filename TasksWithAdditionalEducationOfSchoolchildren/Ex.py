...
#Вам предстоит написать программу, отыскивающую локальные максимумы в трёхмерном списке. 
#Будем считать локальным максимумом элемент, не являющийся крайним , 
#который больше шести своих соседей (спереди, сзади, над, под, слева, справа).
#Программа должна:
#Запросить у пользователя количество слоёв, строк, столбцов.
#Сформировать трёхмерный список заданного размера, заполненный двузначными случайными натуральными числами.
#Вывести список на экран послойно.
#Вывести координаты всех локальных максимумов в списке и их общее количество.

import random as r
r.seed()

l = int(input("Число слоёв: "))
m = int(input("Число строк: "))
n = int(input("Число столбцов: "))

d = [[[r.randint(10,99) for i in range(n)]for j in range(m)]for k in range(l)]

for ll in d:
    print("")
    for mm in ll:
        print(" ".join(str(kk) for kk in mm))
...
for i in range(1, l - 1):
    for j in range(1, m-1):
        for k in range(1, n-1):
            q = d[i][j][k]
            if (q > d[i+1][j][k] and q > d[i-1][j][k] and
                q > d[i][j+1][k] and q > d[i][j-1][k] and
                q > d[i][j][k+1] and q > d[i][j][k+1]):
                nmax += 1
                print("Локальный максимум:", i, j, k)
print("Количество локальных максимумов = ", nmax)
